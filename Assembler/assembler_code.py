...
input_file = input("Enter the address to the input text file: ")
...
for line in data:
    ...
    line = line[:-1]
    line_data = list(filter(lambda x: x, re.split(",| |\(|\)", line)))
    ...
    if line_data[0] in r_type_instructions:
        ...
        line_data[3] = bin(int(line_data[3]))[2:]
        ...
        line_data[2] = bin(int(line_data[2]))[2:]
